			...
			if (newWorker) {
				newWorker.addEventListener('statechange', async () => {
					if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
						// Move hash comparison BEFORE showing the notification
						const hashResponse = await fetch('/service-worker-hash.json');
						if (hashResponse.ok) {
							const hashData = await hashResponse.json();
							...
							if (currentHash !== storedHash) {
								// New service worker is installed and ready to take over, AND hash is different
								console.log('[ServiceWorker] New version ready to be activated');
								if (!notificationShown) {
									notifyUpdateAvailable(registration, currentHash);
