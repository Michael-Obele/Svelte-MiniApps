...
describe('Budget State Management', () => {
	...
	it('should successfully add an expense to a specific budget', () => {
		...
		expect(addedExpense.description, 'Expense description should match input').toBe(
			expenseDescription
		);
	...
	/**
	 * Test: Importing an expense preserves the source timestamp
	 */
	it('should preserve a provided createdAt timestamp when importing an expense', () => {
		budgetState.addBudget('Groceries', 200, 'USD');
		const budgetId = budgetState.budgets.current[0].id;
		const importedDate = '2024-12-01T10:30:00.000Z';

		budgetState.addExpense(budgetId, 'Imported Milk', 5, undefined, importedDate);

		const importedExpense = budgetState.budgets.current[0].expenses[0];
		expect(importedExpense.createdAt).toBe(importedDate);
	});
	...
	it('should correctly update an existing expense with new values', () => {
		...
		const originalCreatedAt = budgetState.budgets.current[0].expenses[0].createdAt;
		...
		expect(updatedExpense.description, 'Expense description should be updated').toBe(
			newDescription
		);
		expect(updatedExpense.amount, 'Expense amount should be updated').toBe(newAmount);
		expect(updatedExpense.createdAt, 'Expense timestamp should remain unchanged').toBe(
			originalCreatedAt
		);
...
describe('Budget State Helper Functions', () => {
	...
	/**
	 * Test: Importing budgets and expenses preserves source timestamps
	 */
	it('should preserve imported timestamps for budgets and expenses', () => {
		const importedBudgetDate = '2024-01-15T08:00:00.000Z';
		const importedExpenseDate = '2024-01-16T09:15:00.000Z';

		budgetState.addBudget('Imported Budget', 100, 'USD', 'budget-1', importedBudgetDate);
		budgetState.addExpense('budget-1', 'Imported Expense', 20, 'expense-1', importedExpenseDate);

		const budget = budgetState.budgets.current[0];
		expect(budget.createdAt).toBe(importedBudgetDate);
		expect(budget.expenses[0].createdAt).toBe(importedExpenseDate);
	});
	...
	it('should handle invalid budget data gracefully', () => {
		...
		expect(budgetState.budgets.current.length, 'Should not add budget with negative amount').toBe(
			0
		);
	...
	it('should handle invalid expense data gracefully', () => {
		...
		expect(
			budgetState.budgets.current[0].expenses.length,
			'Should not add expense with negative amount'
		).toBe(0);
		...
		expect(
			budgetState.budgets.current[0].expenses.length,
			'Should not add expense with empty description'
		).toBe(0);
	});
