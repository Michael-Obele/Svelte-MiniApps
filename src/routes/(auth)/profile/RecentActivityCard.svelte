...
<Card class="relative md:col-span-2">
	<CardHeader>
		...
		<CardDescription>Your latest interactions with Svelte Mini Apps</CardDescription>
	</CardHeader>
	<CardContent>
		<div class="space-y-4">
			{#if recentActivities.length > 0}
				...
				{/each}
			{:else}
				<div class="text-muted-foreground flex items-center justify-center py-4">
					<p>No recent activity yet. Start using apps to see your activity here!</p>
				</div>
			{/if}
		</div>
	</CardContent>
	<CardFooter class="absolute right-0 bottom-0 left-0">
		<Sheet.Root>
