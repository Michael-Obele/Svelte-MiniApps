<script lang="ts">
	...
	let streakMinutes = $derived(getStreakMinutes(attempt));
	...
	let seconds = $derived(
		Math.floor(((currentTime.getTime() - new Date(attempt.lastSmokeDate).getTime()) / 1000) % 60)
	);
</script>
...
<Card class="border-2 border-primary">
	...
	<CardContent>
		...
		<div class="mb-6 grid grid-cols-4 gap-4 text-center">
			<div>
				<div class="text-4xl font-bold text-primary lg:text-5xl">{days}</div>
				<div class="text-sm text-muted-foreground">days</div>
			</div>
			<div>
				<div class="text-4xl font-bold text-primary lg:text-5xl">{hours}</div>
				<div class="text-sm text-muted-foreground">hours</div>
			</div>
			<div>
				<div class="text-4xl font-bold text-primary lg:text-5xl">{minutes}</div>
				<div class="text-sm text-muted-foreground">mins</div>
			</div>
			<div>
				<div class="text-4xl font-bold text-primary lg:text-5xl">{seconds}</div>
				<div class="text-sm text-muted-foreground">secs</div>
			</div>
		</div>
