<script lang="ts">
	...
	let TagIcon = $derived(
		project.tagClass && project.tagClass in tagIcons ? tagIcons[project.tagClass as TagClass] : null
	);
...
<a href={'/apps/' + project.link} class="group block h-full">
	<Card.Root
		class="group border-muted-foreground/10 relative flex h-full flex-col overflow-hidden border-2 transition-all duration-300 hover:scale-[1.02] hover:border-red-500/50 hover:shadow-xl"
	>
		<!-- Category icon overlay -->
		{#if TagIcon}
			<div
				class="from-background to-muted/50 ring-border/50 absolute top-3 right-3 rounded-lg bg-gradient-to-br p-2 shadow-sm ring-1"
			>
				<TagIcon
					class="text-muted-foreground/70 h-4 w-4 transition-colors group-hover:text-red-500"
				/>
			</div>
		{/if}
		<Card.Header class="pb-3">
			...
			<Card.Title
				class="mt-3 text-xl leading-tight font-bold transition-colors group-hover:text-red-600 dark:group-hover:text-red-500"
			>
				{project.title}
			</Card.Title>
		</Card.Header>
		...
		<Card.Footer class="mt-auto flex items-center justify-between border-t pt-4">
			...
			<!-- Difficulty badge -->
			<div
				class={`flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold tracking-wider uppercase ${getDifficultyBg(project.difficulty)} ${getDifficultyColor(project.difficulty)}`}
			>
